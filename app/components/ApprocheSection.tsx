...
export default function ApprocheSection() {
    const features = [
        {
            icon: <Brain size={36} className="text-[#C9A646]" />,
        ...
        {
            icon: <Wallet size={36} className="text-[#C9A646]" />,
        ...
        <section className="relative py-20 px-6 bg-[#0D0D0D] text-white">
            <div className="container mx-auto max-w-6xl">
                ...
                <motion.h2
                    ...
                    viewport={{ once: true }}
                >
                    Plus qu’une <span className="text-[#C9A646]">plateforme</span>, une méthode.
                </motion.h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    {features.map((item, idx) => (
                        <motion.div
                            key={idx}
                            className="relative group bg-gradient-to-b from-[#1A1A1A] to-[#111111] p-8 rounded-2xl border border-[#C9A646]/30 shadow-md hover:shadow-[#C9A646]/40 transition transform hover:-translate-y-2"
                        ...
                        >
                            {/* Glow animé */}
                            <div className="absolute inset-0 rounded-2xl bg-[#C9A646] opacity-0 group-hover:opacity-10 blur-xl transition"></div>

                            {/* Icône */}
                            <div className="mb-6">{item.icon}</div>

                            {/* Titre */}
                            <h3 className="text-xl font-bold mb-4 text-[#C9A646]">{item.title}</h3>
